          ...
          {upcomingBirthdays.length > 0 ? (
            <div className="d-flex flex-column gap-3 py-3 px-3">
              {upcomingBirthdays.map((employee) => (
                <div style={{ borderBottom: "1px solid #E3F4F4" }} className="" key={employee.empID}>
                  <div className="row" style={{ verticalAlign: "middle" }}>
                    <div className="d-flex justify-content-between ">
                      <div className="d-flex gap-2">
                        <div style={{ height: '45px', width: '45px' }}>
                          <img style={{ height: '100%', width: '100%', border: '3px solid gray', borderRadius: '50%', overflow: 'hidden', objectFit: 'cover' }} src={
                        ...
                        <div className="d-flex flex-column my-auto">
                          <span style={{ fontSize: '12px', fontWeight: 'bold' }}>{employee.empID}</span>
                          <span style={{ fontSize: '13px', fontWeight: 'bold' }} className="text-uppercase fw-bold">{`${employee.FirstName} ${employee.LastName}`}</span>
                        </div>
                      </div>
                      <div style={{ fontSize: '13px' }} className="my-auto fw-bolder text-muted">Date <br />
                        {employee.DOB}</div>
                    </div>
                  </div>
                </div>
