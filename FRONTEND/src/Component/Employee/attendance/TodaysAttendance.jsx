...
import Container from '../LeaveComponents/Container';
...
const TodaysAttendance = () => {
    ...
    useEffect(() => {
        ...
        const fetchAttendanceData = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/todays-attendance');
    ...
    return (
        <Container>
            <div className='d-flex justify-content-between py-3'>
                <h4 className='fw-bolder my-auto text-success mb-2'>Today's Attendance</h4>
            ...
            <table className='table table-striped'>
                <thead>
                    <tr>
                        ...
                        <th className='bg-dark text-white py-1'><TbLogin /> Login Time</th>
                        <th className='bg-dark text-white py-1'><TbLogin2 /> Logout Time</th>
                        ...
                        return (
                            <tr key={user.userId}>
                                <td className='fw-bold'>
                                    <div>
                                        <div><img src="" alt="" /></div>
                                        <div><span>{user.empID}</span> <span>{user.name}</span></div>
                                    </div></td>
                                <td className='text-uppercase'>{user.attendance ? user.attendance.loginTime[0] : 'N/A'}</td>
                                <td className='text-uppercase'>{user.attendance ? user.attendance.logoutTime[user.attendance.logoutTime.length - 1] : 'N/A'}</td>
                                {/* <td>{user.attendance ? user.attendance.totalBrake : 'N/A'}</td> */}
                                <td>{calculatetotalBrake(user.attendance ? user.attendance.totalBrake : null)}</td>
                                <td>{calculateTotalLoginTime(user.attendance ? user.attendance.loginTimeMs[0] : null, user.attendance ? user.attendance.logoutTimeMs[user.attendance.logoutTimeMs.length - 1] : null)}</td>
                                ...
                                <td className='text-center'>
                                    <span style={{ fontSize: '.8rem' }} className={`py-1 px-3 rounded-5 shadow-sm fw-bold ${mark === 'Present' ? 'bg-success text-white' : mark === 'Late' ? 'bg-info text-white' : mark === 'Half Day' ? 'bg-warning text-white' : 'bg-danger text-white'}`}>
                                        {mark}
                                    </span>
                                </td>
                                <td className='text-center'>{user.attendance ? user.attendance.breakTime.length : 'N/A'}</td>
                                <td style={{ zIndex: '1' }} className='text-center'><button onMouseEnter={() => setActiveCategory(user)}
                                    onMouseLeave={() => setActiveCategory(null)} className=' btn p-0 fw-bold fs-5 position-relative'><AiOutlineMore /> <span style={{ display: activeCategory === user ? 'flex' : 'none' }}>
                                        <Link to="/GetLogs" style={{ position: 'absolute', whiteSpace: 'pre', right: '70%', bottom: '-100%', zIndex: '5' }} className='shadow p-2 fs-6'>Detailed</Link></span></button> </td>
